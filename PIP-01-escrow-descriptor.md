...
- the operator restricts subsequent `fund_status`, `release`, `refund`, `split`, and `cancel` operations to the recognized participants of that escrow instance
- each participant signs its own operations; threshold release decisions require signatures from the declared threshold of participants
...
- idempotency rules, including the shared correlation value used by repeated operations
- the mechanisms required to successfully `create` an instance and associate participants with it (e.g., pre-declared pubkeys, invitation tokens, or instance IDs)
- timeout-class mappings, including each declared timeout class and its non-`mutual_consent` fallback resolution
...
- `create` — open a new escrow instance or join an existing instance. The authenticated request establishes the instance and associates the participants according to the mechanism defined in the operator's referenced `schema_url`. Repeated calls with the same idempotency key are idempotent and MUST NOT create duplicate escrow instances.
